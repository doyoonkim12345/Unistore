...
export default function EachStore({rtmData = storeTestData, storeData= storeTestData}){
    console.log(rtmData.addressXY)
    return(
            <EachStoreBack color='skyblue'>
                ...
                <span className="time">{`${rtmData.startAm ? 'am' : 'pm'}${rtmData.startTime}시~${rtmData.endAm ? 'am' : 'pm'}${rtmData.endTime}시까지만`}</span>
                </RtmEvents>
                <span className="Always">{storeData.discount}</span>
                <Buttons>
                <a href={`tel:${storeData.phone}`}>전화</a>
                <span>|</span>
                <a href={`https://map.kakao.com/link/to/${storeData.name},${storeData.addressXY&&storeData.addressXY[0]},${storeData.addressXY&&storeData.addressXY[1]}`}>길찾기</a>
                </Buttons>
                </Contents>
            </EachStoreBack> 
    );
